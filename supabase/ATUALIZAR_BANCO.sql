...
-- criar_pedido: mapeia tipo→fluxo (pronto→producao; resto = próprio nome)
create or replace function public.criar_pedido(
  p_numero int,
  p_cliente text,
  p_descricao text default '',
...
-- mover_pedido: conclui na última etapa de qualquer fluxo, exceto 'criacao'
create or replace function public.mover_pedido(
  p_numero int,
  p_etapa_id uuid,
  p_observacao text default '',
...
-- ------------------------------------------------------------
-- 8. CAPACIDADE DA PRODUÇÃO + PERDAS DE MATERIAL
-- ------------------------------------------------------------
-- capacidade (teto) e meta (alvo) por etapa, em peças/dia (0 = não definida)
alter table public.etapas
...
-- ------------------------------------------------------------
-- 9. PLANEJAMENTO DA SEMANA (página "Semana")
-- ------------------------------------------------------------
...
-- setores próprios da página Semana (o admin cria os que usa)
