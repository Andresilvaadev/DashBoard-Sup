...
-- Registra a ocorrência (problema achado na produção) do pedido. Liberado
-- para QUALQUER funcionário ativo: quem vê o problema é quem está na
-- produção. Mexe só nessa coluna, sem dar permissão sobre o resto do pedido.
create or replace function public.registrar_ocorrencia(p_pedido_id uuid, p_texto text)
